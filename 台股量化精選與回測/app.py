...
def run_strategy_bg(strategy_type):
    global status
    with running_lock:
    ...
    try:
        ...
        if strategy_type == 'long':
            stock_robot.run_stock_selection()
        else:
            short_robot.run_stock_selection()
...
def trigger_run(strategy):
    ...
    threading.Thread(target=run_strategy_bg, args=(strategy,), daemon=True).start()
...
def get_history():
    ...
    for f in files:
        ...
        # Match '台股精選標的_YYYYMMDD.csv' or '台股空方精選_YYYYMMDD.csv'
        ...
        if match:
            ...
            try:
                ...
                # Column names for long and short
