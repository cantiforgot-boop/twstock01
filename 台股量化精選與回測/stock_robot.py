...
def get_actual_trading_date_before(offset_days):
    """
    下載台積電歷史資料，取得「100%真實開盤交易日曆」排除颱風假、連假
    """
    try:
...
def evaluate_historical_report_file(target_date_str, prefix, offset_days, stock_map, today_prices):
    ...
    if not os.path.exists(filename):
        return f"  ⚠️ 提示：未找到當初 {date_formatted} 的歷史選股報表。", None, date_formatted
    ...
    try:
        ...
        for idx, row in df.iterrows():
            ...
            if p_today is not None and not np.isnan(p_today):
                gain = (p_today - p_select) / p_select * 100.0
                gains.append(gain)
                track_prices_col.append(round(p_today, 2))
                track_gains_col.append(round(gain, 2))
                sign = "+" if gain >= 0 else ""
                detail_lines.append(f"       - {symbol} {name}\n         ➔ 【{row['選出日期']} 選出價】：{p_select:.2f}\n         ➔ 【{today_date_formatted} 最新價】：{p_today:.2f} ({sign}{gain:.2f}%)")
            else:
        ...
        df[f'T+{offset_days}漲跌幅(%)'] = track_gains_col
        ...
        if gains:
            ...
            return detail_msg, f"{sign}{avg_gain:.2f}%", date_formatted
...
def run_stock_selection():
    ...
    print(f"通過流動性過濾 (5日均量 > 5000張) 的個股共 {len(passed_stage1_df)} 檔")
    ...
    if not passed_stage1_df.empty:
        ...
        for _, row in passed_stage1_df.iterrows():
            ...
            if ticker_df is not None and 'Close' in ticker_df.columns:
                ...
                # 篩選條件：
                # (1) 中長線趨勢：MA20 > MA60
                cond_trend = t_ma20 > t_ma60
                # (2) 極短線強勢：Close > MA5
                cond_price = today_close > t_ma5
                # (3) MACD黃金起漲點：今日柱 > 0 且昨日 <= 0
                cond_macd = (t_hist > 0) and (y_hist <= 0)
                
                if cond_trend and cond_price and cond_macd:
                    # 乖離率與風險提示
                    bias_5d = (today_close - t_ma5) / t_ma5 * 100.0
                    bias_20d = (today_close - t_ma20) / t_ma20 * 100.0
                    
                    risk_alert = "正常"
                    if bias_5d > 3.0 or bias_20d > 10.0:
                        risk_alert = "過熱"
    ...
    # 存檔報表 (統一為 2.0 安全防護格式)
    output_filename = f"data/台股精選標的_{today_str}.csv"
    try:
        if not selected_results:
            df_output = pd.DataFrame(columns=['股票代號', '股票名稱', '5日均量', '風險提示', 'MACD狀態', '選出日期', '選出時收盤價'])
        else:
            df_output = pd.DataFrame(selected_results)
            # 排序
            df_output = df_output.sort_values(by=['風險提示', '選出時收盤價'], ascending=[True, False])
            
        valid_cols = ['股票代號', '股票名稱', '5日均量', '風險提示', 'MACD狀態', '選出日期', '選出時收盤價']
        ...
        print(f"\n報表已成功產出：{os.path.abspath(output_filename)}")
    except Exception as e:
        print(f"輸出報表時出錯: {e}")
    ...
    t5_detail, t5_avg, t5_fdate = evaluate_historical_report_file(t5_target_str, '台股精選標的', 5, stock_map, today_prices)
    t10_detail, t10_avg, t10_fdate = evaluate_historical_report_file(t10_target_str, '台股精選標的', 10, stock_map, today_prices)
    t20_detail, t20_avg, t20_fdate = evaluate_historical_report_file(t20_target_str, '台股精選標的', 20, stock_map, today_prices)
    
    # 5. 合併大訊息 Telegram 播報
    msg = f"📈 【台股今日盤後多方精選名單】 日期：{today_formatted} (5,000張流動性 + 多頭排列)\n"
    msg += "------------------------------------------\n"
    msg += "🔥 【黃金起漲點：MACD 由綠翻紅】\n"
    ...
    if not selected_results:
        ...
        for stock in selected_results:
            msg += f"* {stock['股票代號']} {stock['股票名稱']} | 選出價：{stock['選出時收盤價']} | 5日均量：{stock['5日均量']} | 風險提示：{stock['風險提示']}\n"
            
    msg += "\n------------------------------------------\n"
    msg += f"📊 【多頭策略 T+N 日效益驗收報告】 驗收日期：{today_formatted}\n"
    msg += "📈 【歷史個股後續漲跌名細】\n\n"
    
    # T-5
    msg += f"* 5天前 (當初選出日期：{t5_fdate}) 精選股追蹤：\n"
    msg += f"{t5_detail}\n"
    if t5_avg is not None:
        msg += f"       ➔ 【5天前總平均漲幅】：{t5_avg}\n"
    msg += "\n"
    
    # T-10
    msg += f"* 10天前 (當初選出日期：{t10_fdate}) 精選股追蹤：\n"
    msg += f"{t10_detail}\n"
    if t10_avg is not None:
        msg += f"       ➔ 【10天前總平均漲幅】：{t10_avg}\n"
    msg += "\n"
    
    # T-20
    msg += f"* 20天前 (當初選出日期：{t20_fdate}) 精選股追蹤：\n"
    msg += f"{t20_detail}\n"
    if t20_avg is not None:
        msg += f"       ➔ 【20天前總平均漲幅】：{t20_avg}\n"
    msg += "------------------------------------------"
...
def run_scheduler_loop():
    ...
    排程主循環：每日 18:00 自動觸發
    """
    print("多方選股與回測整合排程機器人已啟動！")
    print("排程設定：每逢台股交易日 (週一至週五) 每日 18:00 自動執行。")
    
    last_run_date = datetime.now().date()
    
    # 防開機補跑邏輯
    now = datetime.now()
    target_today = datetime.combine(now.date(), datetime.min.time()).replace(hour=18, minute=0, second=0)
    if now < target_today:
    ...
    while True:
        try:
            now = datetime.now()
            today = now.date()
            target_today = datetime.combine(today, datetime.min.time()).replace(hour=18, minute=0, second=0)
            
            if now >= target_today and last_run_date != today:
                if now.weekday() < 5:
                    print(f"\n[{now.strftime('%Y-%m-%d %H:%M:%S')}] 觸發交易日多方選股與回測任務...")
                    run_stock_selection()
                else:
