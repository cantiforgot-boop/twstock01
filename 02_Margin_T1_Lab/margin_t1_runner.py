...
# =========================================================================
# Telegram Bot 設定 (沿用主專案帳號)
# =========================================================================
TELEGRAM_BOT_TOKEN = "8948816767:AAHnUGU4eVLHN1U4aELeZbgJtrF-nxpetJE"
TELEGRAM_CHAT_ID = "1057166938"
...
def run_margin_diagnostics(target_date, calendar, stock_map):
    """
    執行指定日期的資券籌碼過濾與股價診斷
    """
    ...
    for c in candidates:
        ...
        if sub_df is not None and not sub_df.empty and 'Close' in sub_df.columns:
            ...
            if not t_rows.empty:
                ...
                # 流動性過濾: 5 日均量 > 1,000 張 (可視需求調整)
                if avg_vol_5d_sheets < 1000.0:
                    continue
...
def main():
    ...
    # 4. 執行 T 日籌碼診斷
    scan_results = run_margin_diagnostics(today_date, calendar, stock_map)
